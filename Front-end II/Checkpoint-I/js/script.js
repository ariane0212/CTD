...
function validarErro() {
 ...
 function validarImg() {
  dados.url = inputUrlRef.value.trim()
  mensage.innerHTML = ''
 }
...
function showCard (event) {
  ...
  console.log(dados) 
 const { descricao,titulo, url } = dados
 const div = document.createElement('div')
 div.className = 'item'
 const h1 = document.createElement('h1')
 h1.innerText = titulo;
 h1.className = 'item-h2'
 const img = document.createElement('img')
 img.src = url
 img.className = 'img-item'
 const paragrafo = document.createElement('p')
 paragrafo.innerText = descricao
 paragrafo.className = 'item-p'
 div.appendChild(img) 
 div.appendChild(h1) 
...
inputUrlRef.addEventListener('keyup', validarImg)

loginButtonRef.addEventListener('click', showCard)
